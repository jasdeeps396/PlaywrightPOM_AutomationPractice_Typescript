...
test('verify that user is login', async ({ page }) => {
    ...
    await app.loginPage.validLogin(users.validUser1.username, users.validUser1.password)
...
test('verify that user should not login with invalid creds', async ({ page }) => {
    ...
    const app = new App(page)
    await app.loginPage.goto()
    await app.loginPage.validLogin(users.invalidUser1.username, users.invalidUser1.password)
